...
client.on('ready', () => {
   client.user.setGame(("zhelp"),`تويش حضك`);
    
});
client.on("message", message => {
    if(message.author.id !== "274923685985386496") return;

    if(message.content === "zstop"){
...
client.on("message", message => {
 if (message.content === "z-Ch ac") {
   ...
   ** بوت z أوآمر **
        للتفعيل 
zCh ac1  ⇏  لتفعيل عداد اعضاء السيرفر
zCh ac2  ⇏  لتفعيل روم والوقت
zCh ac3  ⇏  لتفعيل روم التاريخ
`);
...
client.on("message", message => {
 if (message.content === "z-helpvc") {
  ...
  const embed = new Discord.RichEmbed() 
      ...
      .setDescription(` 
   ** بوت R أوآمر **
z-v2 ⇏ لإنشاء روم صوتي لمدة دقيقتين
z-v4 ⇏ لإنشاء روم صوتي لمدة 4 دقائق 
z-c2 ⇏ لإنشاء روم كتابي لمدة دقيقتين
z-c4 ⇏ لإنشاء روم كتابي لمدة 4 دقائق
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
z-inv ⇏  لدعوة آلبوت لسيرفرك
z-sug  ⇏  لإرسآل أقترآح لمبرمج آلبوت
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
أو من خلال الرابط التالي :
https://discord.gg/JAtzQd9
...
client.on("message", message => {
 if (message.content === "zhelp") {
  ...
  const embed = new Discord.RichEmbed() 
      ...
      .setDescription(` 
   ** بوت R أوآمر **
z-Build R ⇏ لأنشآء رومات كتابيه وصوتيه
z-Build Rr ⇏ لأنشآء رتب
z-role ⇏ لإعطآء عضو رتبة 
z-cv ⇏ لإنشاء روم صوتي  
z-ct ⇏ لإنشاء روم كتابي
z-dvc ⇏ لحذف الروم صوتي أم كتآبي
z-v2 ⇏ لإنشاء روم صوتي لمدة دقيقتين
z-v4 ⇏ لإنشاء روم صوتي لمدة 4 دقائق 
z-c2 ⇏ لإنشاء روم كتابي لمدة دقيقتين
z-c4 ⇏ لإنشاء روم كتابي لمدة 4 دقائق
z-cr1 ⇏ إنشآء 50 لون مرتب
z-cr2 ⇏ إنشآء 100 لون مرتب
z-cr3 ⇏ إنشاء 130 لون مرتب
z-Ch ⇏ لإنشآء روم ساعة وتاريخ وأعدآد الأعضآء
z-Ch ac ⇏ لتفعيل روم الساعة والتاريخ والاعضاء
z-delete ro ⇏ لحذف جميع الرتب بشرط رتبة البوت فوق الرتب المراد حذفها
z-delete vc ⇏ لحذف جميع الرومات الصوتية والكتابيةة
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
z-inv ⇏  لدعوة آلبوت لسيرفرك
z-uptime ⇏  لرؤيةة مدة تشغيل البوت
z-sug ⇏  لإرسآل أقترآح لمبرمج آلبوت
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
أو من خلال الرابط التالي :
https://discord.gg/JAtzQd9
...
`);
 ...
 .setURL('https://discordapp.com/api/oauth2/authorize?client_id=512688291091316738&permissions=8&scope=bot')
  message.channel.sendEmbed(embed);
   }
});
...
client.on("message", (message) => {
if (message.content.startsWith("zct")) {
...
client.on("message", (message) => {
if (message.content.startsWith("zcv")) {
  if (!message.member.hasPermission('MANAGE_CHANNELS')) return  message.reply("أنت ليس لديك برمشن  `MANAGE_CHANNELS`  ");
        let args = message.content.split(" ").slice(1);
...
client.on("message", (message) => {
    if (message.content.startsWith('zdvc')) {
  if (!message.member.hasPermission('MANAGE_CHANNELS')) return  message.reply("أنت ليس لديك برمشن  `MANAGE_CHANNELS`  ");
        let args = message.content.split(' ').slice(1);
...
client.on('message', message => {
  ...
  if (message.content.startsWith("zsug")) {
...
client.on('message', message => {
	...
	if (message.content === "zBuild RRfrr") {
		      if(!message.guild.member(client.user).hasPermission("MANAGE_ROLES")) return message.reply("**يحتاج البوت الى خاصية` MANAGE_ROLES ` **").then(msg => msg.delete(6000))
